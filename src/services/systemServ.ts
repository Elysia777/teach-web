...
import { type DataResponse, type OptionItem, type TreeNode } from '@/models/general'
//获取角色选择项列表后台数据请求方法
...
//获取字典选择项列表后台数据请求方法
export async function getDictionaryOptionItemList(code: string): Promise<OptionItem[]> {
...
//菜单保存后台数据请求方法
export async function menuSave(editType: number, node: TreeNode): Promise<DataResponse> {
