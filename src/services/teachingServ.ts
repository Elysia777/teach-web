...
//获取课程数据表后台数据请求方法
export async function getCourseList(numName: string | null): Promise<CourseItem[]> {
...
//获取当前学生已选课程
export async function getStudentCourseList(userId: number | null,numName:string|null): Promise<CourseItem[]> {
  const res = await generalRequest('/api/course/getStudentCourseList', {
    userId: userId,
    numName:numName
  })
  return res.data as CourseItem[]
...
//保存学生选课
export async function studentCourseSave(userId:number,courseId:number) :Promise<DataResponse>{
  const res =await generalRequest('/api/course/studentCourseSave',{
    userId:userId,
    courseId:courseId
  })
  return res as DataResponse
...
//删除学生选课
export async function studentCourseDelete(courseId:number,userId:number):Promise<DataResponse>{
  const res=await generalRequest('/api/course/studentCourseDelete',{
    courseId:courseId,
    userId:userId
  })
  return res as DataResponse
...
//获取老师任课的方法
export async function getTeacherCourseList(userId:number,numName:string|null): Promise<[]> {
  const res = await generalRequest('/api/course/getTeacherCourse', {
    userId:userId,
    numName:numName
  })
  return res.data as []
...
//获取成绩列表后台数据请求方法
export async function getScoreList(studentId: number | null, courseId: number | null): Promise<[]> {
...
export async function getMaterialCourseItemOptionList(role: string): Promise<OptionItem[]> {
  const res = await generalRequest('/api/material/getMaterialCourseItemOptionList', { role: role })
  return res.itemList as OptionItem[]
}

export async function getMaterialTreeNodeList(courseId: number): Promise<TreeNode[]> {
...
export async function materialSave(courseId: number, node: TreeNode): Promise<DataResponse> {
...
export async function importStudentExcl(currentTaskId: number | null, file: any): Promise<any> {
...
export async function deleteTaskStudent(taskStudentId: number): Promise<DataResponse> {
...
export async function getTeamOptionItemList(currentTaskId: number | null): Promise<OptionItem[]> {
...
export async function getStudentTeamInfo(taskStudentId: number | null): Promise<DataResponse> {
...
export async function setStudentTeam(taskStudentId: number, teamId: number): Promise<DataResponse> {
...
export async function uploadTeamFile(
  ...
  formData.append('file', file)
  const res = await uploadRequest(
    '/api/team/uploadTeamFileWeb?remoteFile=' + remoteFile + '&taskStudentId=' + taskStudentId,
    formData
  )
  return res as DataResponse
}

export async function getTeamManageData(currentTaskId: number | null): Promise<DataResponse> {
  const res = await generalRequest('/api/team/getTeamManageData', {
    currentTaskId: currentTaskId
  })
  return res as DataResponse
}
export async function getTeamDataList(currentTaskId: number | null): Promise<TermItem[]> {
...
export async function saveTeamScore(type: number, dataList: TermItem[]): Promise<DataResponse> {
