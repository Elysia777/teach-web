...
//获取学生个人简介信息后台数据请求方法
export async function getStudentIntroduceData(studentId: number | null): Promise<DataResponse> {
...
//获取学生照片数据后台数据请求方法
export async function getPhotoImageStr(fileName: String): Promise<DataResponse> {
...
export async function uploadPhoto(remoteFile: string, file: any): Promise<any> {
  ...
  const res = await uploadRequest('/api/base/uploadPhotoWeb?remoteFile=' + remoteFile, formData)
  return res as DataResponse
}
