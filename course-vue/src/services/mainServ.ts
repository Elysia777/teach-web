...
import { type MenuInfo, type DataResponse, type ValidateCode } from '@/models/general'
...
export async function getValidateCode(): Promise<ValidateCode> {
  const res = await generalRequest('/api/auth/getValidateCode', null)
  return res.data as ValidateCode
}
//后台检验验证码数据请求方法
export function testValidateInfo(data: Object): Promise<DataResponse> {
  const res = generalRequest('/api/auth/testValidateInfo', data)
  return res as Promise<DataResponse>
}
//重置密码后台数据请求方法
export function resetPassWord(data: Object) {
  return generalRequest('/api/auth/resetPassWord', data)
}
//用户注册后台数据请求方法
export function registerUser(data: Object) {
  return generalRequest('/api/auth/registerUser', data)
}
//获取主页面展示数据的后台数据请求方法
