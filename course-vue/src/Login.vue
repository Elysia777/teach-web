              ...
              <div>
                <div class="main5 flex-col">
                  <div class="wrap2 flex-row justify-between">
                    ...
                    <input class="inputWidth" v-model="username" placeholder="请输入账号" />
                  </div>
                </div>
                ...
                <div class="main8 flex-col">
                  <div class="wrap4 flex-row">
                    ...
                    <input class="codeWidth" v-model="valiCode" placeholder="请输入的验证码" />
                    <img
...
import { getValidateCode, testValidateInfo, resetPassWord, registerUser } from '@/services/mainServ'
...
export default defineComponent({
  ...
  async created() {
    const localForm = this.$cookies.get(USER_KEY)
    if (localForm) {
      this.remember = true
      //解码
      this.username = Base64.decode(localForm.username)
      this.password = Base64.decode(localForm.password)
    } else {
      this.remember = false
    }
  ...
  methods: {
    ...
    async loginSubmit() {
      ...
      if (this.username == '' || this.username == undefined) {
        ...
        message(this, '密码为空')
      } //else if (this.valiCode == '' || this.valiCode == undefined) {
        //message(this, '验证码为空')} 
        else {
